...
const RGPDView = () => {
  ...
  return (
    <div className='flex flex-col text-justify justify-center items-center py-3 gap-y-8 px-6 rounded-xl bish-bg-white transition-all duration-500 hover:bish-border-blue'>
      <h1 className='bish-text-blue'>Protection de vos données</h1>
      <p className='w-5/6'>Bish se réserve la possibilité de modifier, à tout moment, en tout ou partie sa politique de données personnelles. Vous êtes donc invités à consulter régulièrement cette page afin de prendre connaissance des changements éventuels effectués. Si ces changements sont impactant pour vous, alors nous vous les notifierons directement (par email, en magasin ou sur Bish.com).</p>
        
        <h5>La collecte des données:</h5>
        <div className=' w-5/6 border-solid border-2 bish-border-gray py-3 px-6 rounded-xl bish-bg-white transition-all duration-500 hover:bish-border-blue'>
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(1)}>
              <label className='font-medium my-2'>Quelques définitions</label>
              <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[1] ? 'rotate-180':''} h-6`} />
          </button>
            ...
            <ul>
              ...
              <li>
                ...
                <h6>- L’organisation de jeux concours</h6>
                  <p><span>Les données traitées : </span>Civilité, nom, prénom, date de naissance, email, téléphone, photo (si casting), Opt-in Newsletter Bish ou Opt-in du partenaire du jeu concours nommé, et gain</p>
                ...
                <h6>- La création et la gestion du compte sur la Plateforme Communautaire Bish</h6>
              ...
              <li>
                <h5>Traitements opérés sur la base légale de de l'intérêt légitime poursuivi par Bish</h5>
                ...
                <p>Bish peut utiliser les données vous concernant afin de personnaliser la publicité qui vous sera diffusée sur son site ou sur des sites externes (exemple : sur les réseaux sociaux), pour le compte d’annonceurs tiers et sous réserve que vous ayez donné votre consentement aux cookies publicitaires. Aucune donnée personnelle n’est transmise aux annonceurs concernés.<br></br>A cette fin, Bish se fonde sur son intérêt légitime d’améliorer sa connaissance client pour établir un profil vous concernant et ainsi personnaliser les publicités grâce à la combinaison de vos données de navigation et des données que vous fournissez à l’occasion de vos interactions avec Bish (vous pouvez vous opposer à ce traitement de données en nous adressant un email à dataprotectionofficer@bish.com).<br></br>Bish peut utiliser dans ce cadre les services de son partenaire VALIUZ, agissant en qualité de responsable conjoint de traitement (cliquez ici vers la politique de confidentialité VALIUZ).</p>
              </li>
              <li>
        ...
        <h5>Les principaux moments de collecte de données:</h5>
        <div className='w-5/6 border-solid border-2 bish-border-gray py-3 px-6 rounded-xl bish-bg-white transition-all duration-500 hover:bish-border-blue'>
          ...
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(5)}>
              <label className='font-medium my-2'>Collecte au travers du programme de fidélité Bish</label>
              <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[5] ? 'rotate-180':''} h-6`} />
          </button>
          ...
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(6)}>
              <label className='font-medium my-2'>Collecte de données en magasin</label>
              <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[6] ? 'rotate-180':''} h-6`} />
          </button>
          ...
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(7)}>
              <label className='font-medium my-2'>Collecte sur le site Bish.com ou sur l’application mobile Bish</label>
              <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[7] ? 'rotate-180':''} h-6`} />
          </button>
          <p className={isOpen[7] ? "block" : "hidden"}>
            Pour que vous puissiez réaliser des achats sur bish.com nous devons recueillir certaines de vos informations afin de pouvoir gérer votre commande, telles que votre nom, prénom ou encore votre adresse de livraison. Certaines de ces informations sont obligatoires et sont marquées d'un astérisque. Nous collectons également des données au fil de vos achats nous permettant d’analyser vos préférences. Une information sur le traitement effectué et sur vos droits vous sera donnée au moment de la collecte.
          </p>
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(8)}>
              <label className='font-medium my-2'>Collecte sur la Plateforme Communautés</label>
              <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[8] ? 'rotate-180':''} h-6`} />
          </button>
          <p className={isOpen[8] ? "block" : "hidden"}>
          </p>
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(9)}>
              <label className='font-medium my-2'>Collecte au travers du programme de fidélité Bish</label>
              <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[9] ? 'rotate-180':''} h-6`} />
          </button>
          ...
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(10)}>
              <label className='font-medium my-2'>Collecte lors de jeux concours</label>
              <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[10] ? 'rotate-180':''} h-6`} />
          </button>
          ...
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(11)}>
              <label className='font-medium my-2'>Collecte indirecte</label>
              <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[11] ? 'rotate-180':''} h-6`} />
          </button>
          ...
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(12)}>
              <label className='font-medium my-2'>Collecte lors de votre navigation sur le site</label>
              <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[12] ? 'rotate-180':''} h-6`} />
          </button>
          ...
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(13)}>
              <label className='font-medium my-2'>Collecte lors de votre dépôt de candidature pour un poste au sein du Groupe Bish</label>
              <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[13] ? 'rotate-180':''} h-6`} />
          </button>
        ...
        <h5>L'usage des données:</h5>
        <div className='w-5/6 border-solid border-2 bish-border-gray py-3 px-6 rounded-xl bish-bg-white transition-all duration-500 hover:bish-border-blue'>
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(14)}>
                <label className='font-medium my-2'>La sécurité de la navigation</label>
                <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[14] ? 'rotate-180':''} h-6`} />
          </button>
            <p className={isOpen[14] ? "block" : "hidden"}>
              Toutes les pages de www.bish.com sont sécurisées et cryptées, notamment les pages de collecte de vos données. Vous pouvez le vérifier en regardant l'adresse de la page : un cadenas apparaît avant ou après le mot Bish (Bish EUROPE) [FR] ; selon votre navigateur.
            </p>
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(14)}>
                <label className='font-medium my-2'>La sécurité de la navigation</label>
                <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[14] ? 'rotate-180':''} h-6`} />
          ...
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(15)}>
                <label className='font-medium my-2'>La sécurité de stockage des données</label>
                <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[15] ? 'rotate-180':''} h-6`} />
          </button>
          ...
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(16)}>
                <label className='font-medium my-2'>Notre usage</label>
                <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[16] ? 'rotate-180':''} h-6`} />
          </button>
            <p className={isOpen[16] ? "block" : "hidden"}>
              <h5>Le destinataire de vos données:</h5>
              <ul>
          ...
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(17)}>
                <label className='font-medium my-2'>Client situés à la Réunion ayant adhéré au programme de fidelite</label>
                <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[17] ? 'rotate-180':''} h-6`} />
          </button>
            <p className={isOpen[17] ? "block" : "hidden"}>
              Afin de vous offrir le meilleur service et une utilisation de la carte de fidélité dans les magasins Bish de nos franchisés dans les DOM-TOM et sur www.bish.com ; dans les cas de création de carte ou d’utilisation de carte en magasin, la société de notre franchisé, transmettra les données à caractère personnel collectées grâce au système fidélité à Bish. A l’inverse, dans les cas de création de carte ou d’utilisation de carte sur www.bish.com, Bish, transmettra les données à caractère personnel collectées grâce au système fidélité, à la société de notre franchisé Vous pouvez vous opposer à ce transfert à l’adresse suivante : dataprotectionofficer@bish.com . Ces transferts de données se feront de manière sécurisée. La société franchisée et Bish seront ainsi chacune responsable du traitement de vos données sur leurs bases de données respectives.
              Pour la Réunion :
              Pour exercer vos droits d'accès, de rectification, de portabilité, de limitation, d’effacement, de transmission de directives anticipées et d’opposition aux traitements des données vous concernant auprès de la société Ravate Tissus, société franchisée de Bish, contactez 0262 23 49 00 ou supmarketing.bish@ravate-tissus.fr .
              Pour la Guadeloupe, Martinique et Guyane : Pour exercer vos droits d'accès, de rectification, de portabilité, de limitation, d’effacement, de transmission de directives anticipées et d’opposition aux traitements des données vous concernant. Contactez marketing.bish@sharko.fr
            </p>
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(18)}>
                <label className='font-medium my-2'>L'alliance VALIUZ</label>
                <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[18] ? 'rotate-180':''} h-6`} />
          </button>
            <p className={isOpen[18] ? "block" : "hidden"}>
              L’alliance VALIUZ
              Bish est membre de l’alliance VALIUZ.
              ...
              VALIUZ est une société codétenue par Bish. Pour consulter la liste des entreprises participantes de l’alliance, cliquez ici.
        ...
        <h5>La publicité chez Bish:</h5>
        <div className='w-5/6 border-solid border-2 bish-border-gray py-3 px-6 rounded-xl bish-bg-white transition-all duration-500 hover:bish-border-blue'>
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(19)}>
                <label className='font-medium my-2'>Courrier papiers</label>
                <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[19] ? 'rotate-180':''} h-6`} />
          </button>
          ...
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(20)}>
                <label className='font-medium my-2'>Email et sms</label>
                <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[20] ? 'rotate-180':''} h-6`} />
          </button>
          ...
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(21)}>
                <label className='font-medium my-2'>Utilisation des réseaux sociaux et google</label>
                <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[21] ? 'rotate-180':''} h-6`} />
          </button>
          <p className={isOpen[21] ? "block" : "hidden"}>
            ...
            - Bish utilise également un service proposé par les réseaux sociaux tels que Facebook ainsi que par Google, permettant d’exposer des publicités Bish sur le réseau social ou sur Google à une population de personnes déterminée selon les critères d’activité chez Bish. Par exemple, Bish pourrait demander à Facebook d’exposer à la publicité ses clients qui ont acheté dans le rayon femme dans les 3 derniers mois
            ...
            - Vous pouvez exercer votre droit d’opposition à faire partie de mesures de performance à l’adresse suivante : dataprotectionoffier@bish.com
          </p>
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(22)}>
                <label className='font-medium my-2'>Notification push</label>
                <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[22] ? 'rotate-180':''} h-6`} />
          </button>
          ...
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(23)}>
                <label className='font-medium my-2'>Contact non publicitaire</label>
                <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[23] ? 'rotate-180':''} h-6`} />
          </button>
        ...
        <div className='w-5/6 border-solid border-2 bish-border-gray py-3 px-6 rounded-xl bish-bg-white transition-all duration-500 hover:bish-border-blue'>
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(24)}>
            <label className='font-medium my-2'>Le détail de vos droits</label>
            <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[24] ? 'rotate-180':''} h-6`} />
          </button>
          <p className={isOpen[24] ? "block" : "hidden"}>
            Le droit d'accès : : Il s’agit du droit d'obtenir la communication de toutes les informations vous concernant détenues par Bish.

            Le droit de rectification : Il s’agit du droit d'obtenir la rectification des informations inexactes vous concernant détenues par Bish.

            Le droit à l’oubli: Il s’agit du droit d'obtenir la suppression des informations vous concernant détenues par Bish. Toutefois, Bish ne sera pas tenu d’effacer les données qu’elle doit conserver dans le cadre d’une obligation légale (ex : les données de facturation).
          ...
          <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(25)}>
                <label className='font-medium my-2'>L'exercice de vos droits</label>
                <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[25] ? 'rotate-180':''} h-6`} />
          </button>
          <p className={isOpen[25] ? "block" : "hidden"}>
            Vous pouvez exercer vos droits par email à contact@bish.com et en cas de difficultés à dataprotectionofficer@bish.com ou en écrivant à Bish – D.P.O - 100 rue du Calvaire, HEM (59510) France. Une réponse vous sera alors adressée dans un délai de 1 mois suivant la réception de la demande.
        ...
        <div className='w-5/6 border-solid border-2 bish-border-gray py-3 px-6 rounded-xl bish-bg-white transition-all duration-500 hover:bish-border-blue'>
            <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(26)}>
                  <label className='font-medium my-2'>La définition</label>
                  <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[26] ? 'rotate-180':''} h-6`} />
            </button>
            ...
            <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(27)}>
              <label className='font-medium my-2'>Les données collectées</label>
              <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[27] ? 'rotate-180':''} h-6`} />
            </button>
            ...
            <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(28)}>
              <label className='font-medium my-2'>La maitrise des cookies</label>
              <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[28] ? 'rotate-180':''} h-6`} />
            </button>
            ...
            <button className='flex justify-between w-full' id="div1" onClick={() => handleClick(29)}>
              <label className='font-medium my-2'>Le cookie de l'alliance VALIUZ</label>
              <img src={arrow} alt="Dérouler la réponse" className={`${isOpen[29] ? 'rotate-180':''} h-6`} />
            </button>
