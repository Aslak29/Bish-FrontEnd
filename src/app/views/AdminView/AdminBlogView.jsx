...
const AdminBlogView = () => {
    ...
    useEffect(() => {
        ...
        respArr[0].data.map((res) => setRows(current => [...current, [
            res.id,
            res.title,
            res.description,
            <img className='object-contain h-10 m-auto hover:absolute hover:scale-[10.0] hover:z-50' src={window.location.origin + '/src/app/assets/images/blog/' + res.pathImage} alt={res.name}/>,
        ...
        setFormCreate(
            ...
            >
                <Form className="grid grid-cols-2 sm:grid-cols-4 gap-4">        
                        ...
                        {/* Description */}
                        <div className="flex flex-col col-span-2 row-span-2">
                            <span>Rédigez l'article ici</span>
                            <Field className='h-full' as="textarea" type="text" name="description" required/>
                        </div>
                        {/* Button Modifier */}
              ...
              <table className="table-fixed w-full pl-5 mt-20" id="searchTable">
                ...
                <thead className='border-b-4 bish-border-gray sticky top-40 bish-bg-white shadow'>
                  <tr>
                    ...
                    <th className={labelHeader} title='Tendance'>Titre</th>
                    <th onClick={() => sort(3)}>
                      <div className={headSort}>
