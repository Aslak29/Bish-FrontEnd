...
import {
  URL_BACK_CREATE_PRODUCT,
  URL_BACK_TYPE_TAILLE,
...
const FormCreate = (props) => {
  ...
  const createRow = (values) => {
    if (window.confirm("Êtes-vous sûr de vouloir ajouter le produit ?")) {
      apiBackEnd
        .post(
          `${URL_BACK_CREATE_PRODUCT}${values.name}/${values.description}/${values.infoFile.name}/${values.categorie}/${values.promotion}/${values.price}/${values.trend}/${values.available}/${values.stock.xs}/${values.stock.s}/${values.stock.m}/${values.stock.l}/${values.stock.xl}/`
        )
        .then((res) => {
          if (res.status === 200) {
            ...
            toast.success(
              `Produit ${res.data.id} - ${res.data.name} ajouté !`,
              {
                position: "top-right",
                autoClose: 5000,
  ...
  return (
    <Formik
      initialValues={productCreateInitialValues(stockAdulte)}
      ...
      {(formikProps) => (
        <Form className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <div className="preview row-span-4 h-96 shadow-lg">
            <img
              className="hidden object-contain h-full w-full"
              id="img-preview"
          ...
          <div className="flex flex-row h-20 col-span-2">
            {formikProps.values.Type === "Adulte" &&
              Object.entries(stockAdulte).map(([key, value]) => (
                <div className="flex flex-col w-1/5 h-full" key={key}>
                  <span>{key.toUpperCase()}</span>
                  <Field
                    className="h-full"
                    type="number"
                    name={"stock." + key.toLowerCase()}
                    required
                  />
                </div>
              ))}
          ...
          <div className="flex flex-col h-20 col-span-2">
            ...
            <Field className="h-full" name="promotion" as="select">
              <option value="-">-</option>
              {props.promotions.data.map((resPromo) => (
                <option key={resPromo.id} value={resPromo.id}>
                  {resPromo.remise} % -{" "}
                  {new Date(resPromo.start_date).toLocaleDateString("fr")}{" "}
                  {new Date(resPromo.start_date).toLocaleTimeString([], {
                    hour: "2-digit",
                    minute: "2-digit",
                  })}{" "}
                  - {new Date(resPromo.end_date).toLocaleDateString("fr")}{" "}
                  {new Date(resPromo.end_date).toLocaleTimeString([], {
                    hour: "2-digit",
                    minute: "2-digit",
                  })}{" "}
                  {resPromo.name && " - " + resPromo.name}
                </option>
              ))}
            </Field>
          </div>
          ...
          <div className="flex flex-col h-20">
            ...
            <Field className="h-full" name="categorie" as="select" required>
              {props.categories.data.map((resCateg) => (
                <option key={resCateg.id} value={resCateg.id}>
                  {resCateg.name}
                </option>
              ))}
            </Field>
          ...
          <div className="flex flex-row h-20 justify-around">
            <div className="flex flex-col h-full justify-center align-items-center">
              <span>Tendance</span>
              <Field
                className="h-8 w-8 lg:h-10 lg:w-10 bish-text-blue m-auto"
                type="checkbox"
                name="trend"
              />
            </div>
          ...
          <button
            type="submit"
            className="bish-bg-blue py-3 w-full bish-text-white col-span-4 mx-auto"
          >
            Ajouter
          </button>
        </Form>
